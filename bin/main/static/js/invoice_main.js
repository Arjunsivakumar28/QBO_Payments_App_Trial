...
function displayRows() {
    ...
    for (idx in apiArray) {
        ...
        if (idx >= start && idx < end) {
            ...
            if (apiArray[idx].balance == 0) { pay_btn = ""; } else { pay_btn = `<button class="btn btn-outline-success z-2" onclick="payInvoice(` + idx + `)">Pay</button>`; }
...
function deleteFirstInvoice() {
    ...
    `
    document.getElementById("invoice-table-body").innerHTML = load_html;
    fetch("http://localhost:8080/deleteInvoice")
        .then(response => response.text())
        .then(text => {
            let data = text.split(" /// ");
            for (let i = 0; i < data.length; i++) {
...
function createInvoice() {
    ...
    `
    document.getElementById("invoice-table-body").innerHTML = load_html;
    fetch("http://localhost:8080/createInvoice")
        .then(response => response.text())
        .then(text => {
            let data = text.split(" /// ");
            for (let i = 0; i < data.length; i++) {
...
function payInvoice(idx) {
    ...
    fetch("http://localhost:8080/payInvoice?id=" + apiArray[idx].id)
        .then(response => response.text())
        .then(text => {
...
function tableColSort(ele) {
    ...
    if (attri == "invoice no.") { attri = "id"; } else if (attri == "due date") { attri = "dueDate"; }
