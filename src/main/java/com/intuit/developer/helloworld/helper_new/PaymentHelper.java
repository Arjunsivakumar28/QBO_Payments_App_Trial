...
public final class PaymentHelper {
	...
	public static Payment getPaymentForInvoice(DataService service, Invoice invoice) throws FMSException, ParseException {
		...
		Line line1 = new Line();
		line1.setAmount(invoice.getBalance());
		line1.setDetailType(LineDetailTypeEnum.ACCOUNT_BASED_EXPENSE_LINE_DETAIL);
		...
		payment.setPaymentType(PaymentTypeEnum.CREDIT_CARD);
		payment.setTotalAmt(invoice.getBalance());
		payment.setUnappliedAmt(new BigDecimal("0"));
		return payment;
	}
	
	public static Payment getPayments(DataService service) throws FMSException, ParseException {
		List<Payment> payments = (List<Payment>) service.findAll(new Payment());
		if (!payments.isEmpty()) {
			return payments.get(0);
		}
		return service.add(getCheckPaymentFields(service));
	}
